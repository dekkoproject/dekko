...
do
    ...
    mkdir ./${BUILD_DIR}
    cd ./${BUILD_DIR}
    sudo click chroot -a $ARCH -f ${CLICK_SDK_FRAMEWORK} run cmake ..
    sudo click chroot -a $ARCH -f ${CLICK_SDK_FRAMEWORK} run make
    sudo click chroot -a $ARCH -f ${CLICK_SDK_FRAMEWORK} run make DESTDIR=${FAT_DIR} install
    cd ../
