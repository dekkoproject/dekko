...
void SmtpClient::setAccount(const QMailAccountId &id)
{
    ...
    QMailAccountConfiguration::ServiceConfiguration c = config.serviceConfiguration("smtp");
    ...
    Q_FOREACH(const QString &service, UOAPluginFactory::instance()->services()) {
        if (service == authPlugin) {
            m_service = UOAPluginFactory::instance()->createService(service, uoaId);
            break;
        }
    }
    if (m_service) {
        m_service->setProtocol("smtp");
    } else {
        qMailLog(SMTP) << "Failed to create UOA service";
    }
#endif
}
...
void SmtpClient::nextAction(const QString &response)
{
    ...
    {
#ifdef UOA_ENABLED
        m_service->setCapabilities(capabilities);
#endif
...
#elif defined(UOA_ENABLED)
        QFuture<QByteArray> initialResponse;
        if (m_service->name() == QStringLiteral("sasl")) {
            SaslPluginNS::SaslData saslData = m_service->sessionData().data<SaslPluginNS::SaslData>();
            saslData.setService("smtp");
            saslData.setFqdn(config.serviceConfiguration("smtp").value(QStringLiteral("server")));
            saslData.setRealm(saslData.Fqdn());
            QString caps = SmtpAuthenticator::getSaslCapabilities(config.serviceConfiguration("smtp"), capabilities);
            initialResponse = QtConcurrent::run(m_service, &UOAService::process, saslData, caps);
        } else {
            OAuth2PluginNS::OAuth2PluginData oauthData = m_service->sessionData().data<OAuth2PluginNS::OAuth2PluginData>();
    //        oauthData.setClientId("");
    //        oauthData.setClientSecret("");
            initialResponse = QtConcurrent::run(m_service, &UOAService::process, oauthData, QString("user_agent"));
        }
        initialResponse.waitForFinished();
        QByteArray ir = initialResponse.result();
        if (!ir.isEmpty()) {
            sendCommand(ir);
            status = Authenticating;
        } else {
...
#ifdef UOA_ENABLED
            QFuture<QByteArray> f = QtConcurrent::run(m_service, &UOAService::processServerResponse, QByteArray::fromBase64(response.toLatin1()));
            f.waitForFinished();
            QByteArray r = f.result();
            if (!r.isEmpty()) {
                sendCommand(r, true);
