...
#if !defined(SNAP) && !defined(IS_UNITY8)
...
{
    static QString path(lockDir() + QString("messageserver-instance.lock"));
    //check unix path
#ifdef Q_OS_UNIX
    //Store the file in /var/lock instead system's temporary directory
#if !defined(SNAP) && !defined(IS_UNITY8)
...
{
#if defined(IS_UNITY8)
    static const QString path = QDir::homePath() + QStringLiteral("/.config/dekko.dekkoproject/tmp/");
    if (!QDir(path).exists()) {
        QDir().mkpath(path);
    }
    return path;
