...
void ImapService::initiatePushEmail()
{
    ...
    if (_networkSession.isNull()) {
        qMailLog(Messaging) << "Creating new idle session";
        createIdleSession();
        return;
    }
#else
...
#ifdef USE_CONNECTIVITY_API
    if (_networkSession.isNull()) {
        qMailLog(Messaging) << "Creating new Connectivity network session for account" << _accountId;
        _networkSession = QSharedPointer<Connectivity>(new Connectivity, &QObject::deleteLater);
        Q_ASSERT(_networkSession.data());
        connect(_networkSession.data(), &Connectivity::onlineUpdated, this, &ImapService::onOnlineStateChanged);
...
#ifdef USE_CONNECTIVITY_API
    qMailLog(Messaging) << "IDLE session opened";
    connect(_networkSession.data(), SIGNAL(statusUpdated(Connectivity::Status)), this,
            SLOT(onSessionStateChanged(Connectivity::Status)));
#else
...
#ifdef USE_CONNECTIVITY_API
void ImapService::onSessionStateChanged(Connectivity::Status status)
{
    bool sessionError = false;
    switch(status) {
    case Connectivity::Status::Offline:
        qMailLog(Messaging) << "IDLE session error: Connectivity::Status::Offline";
        sessionError = true;
        break;
    case Connectivity::Status::Connecting:
        qMailLog(Messaging) << "IDLE session error: Connectivity::Status::Connecting";
        sessionError = true;
        break;
    case Connectivity::Status::Online:
        qMailLog(Messaging) << "IDLE session connected: Connectivity::Status::Online";
        break;
    }
...
void ImapService::onSessionConnectionTimeout()
{
    ...
    if (!_networkSession.isNull()) {
        if (!_networkSession->online()) {
            ...
            onSessionError(QNetworkSession::UnknownSessionError);
        }
#else
    if (_networkSession) {
