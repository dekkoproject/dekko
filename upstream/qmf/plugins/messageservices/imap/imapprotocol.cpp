...
#ifdef UOA_ENABLED
#include <uoa/UOAPluginFactory.h>
#include <uoa/UOAService.h>
...
public:
#ifdef UOA_ENABLED
    LoginState() : ImapState(IMAP_Login, "Login"), _service(0) { LoginState::init(); }
#else
...
void LoginState::setConfiguration(const QMailAccountConfiguration &config, const QStringList &capabilities)
{
    ...
    _capabilities = capabilities;
#ifdef UOA_ENABLED
    QMailAccountConfiguration::ServiceConfiguration c = _config.serviceConfiguration("imap4");
    QString authPlugin = c.value(QStringLiteral("uoaPlugin"));
    quint32 uoaId = c.value("uoaId").toUInt();
    Q_FOREACH(const QString &service, UOAPluginFactory::instance()->services()) {
        if (service == authPlugin) {
            _service = UOAPluginFactory::instance()->createService(service, uoaId);
            break;
        }
    }
    if (_service) {
        _service->setCapabilities(_capabilities);
        _service->setProtocol("imap");
        _service->setServiceConfig(_config.serviceConfiguration("imap4"));
    } else {
        qWarning() << "Failed to create UOA service";
    }
#endif
}
#endif
...
#ifdef UOA_ENABLED
    // Don't puke when you look at this. This isn't ideal at all!
    // But it's going to be an intrusive change to the entire ImapProtocol/ImapContext logic
    // in order to get this working asynchronously with the signon plugins.
    // The other short term option here is to use a QEventLoop and spin that until we receive the sessiondata
    // but thats probably a bad idea. It's better to stick it on another thread and wait for the response.
    // Yes it will block here but it;s the best we can do right now.
    QFuture<QByteArray> initialResponse;
    if (_service->name() == QStringLiteral("sasl")) {
        SaslPluginNS::SaslData saslData = _service->sessionData().data<SaslPluginNS::SaslData>();
        saslData.setService("imap");
        saslData.setFqdn(_config.serviceConfiguration("imap4").value(QStringLiteral("server")));
        saslData.setRealm(saslData.Fqdn());
        QString caps = ImapAuthenticator::getSaslCapabilities(_config.serviceConfiguration("imap4"), _capabilities);
        initialResponse = QtConcurrent::run(_service, &UOAService::process, saslData, caps);
    } else {
        OAuth2PluginNS::OAuth2PluginData oauthData = _service->sessionData().data<OAuth2PluginNS::OAuth2PluginData>();
//        oauthData.setClientId("");
//        oauthData.setClientSecret("");
        initialResponse = QtConcurrent::run(_service, &UOAService::process, oauthData, QString("user_agent"));
    }
    initialResponse.waitForFinished();
    return c->sendCommand(initialResponse.result());
#else
...
#ifdef UOA_ENABLED
    QFuture<QByteArray> f = QtConcurrent::run(_service, &UOAService::processServerResponse, challenge);
    f.waitForFinished();
    QByteArray response = f.result();
    if (!response.isEmpty()) {
        c->sendData(response, true);
    } else {
        c->sendData("");
    }
#else
