...
bool SettingsFileBase::setPath(const QString &path)
{
    ...
    if (!dir.exists() && !dir.mkpath(QStringLiteral("."))) {
        setErrorString(QStringLiteral("Cannot create directory: %1").arg(dir.path()));
        return false;
    }
...
bool SettingsFileBase::readFile()
{
    ...
    if (!doc.isObject()) {
        setErrorString(QStringLiteral("Unsupported config file type. Expecting an object here."));
        return false;
    }
...
bool SettingsFileBase::writeFile()
{
    ...
    if (data.isEmpty() && !document.isEmpty()) {
        setErrorString(QStringLiteral("Encoding failure"));
        return false;
    }
